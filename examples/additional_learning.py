...
def run_additional_learning_example():
    ...
    auto_model = Fedot(problem=problem, seed=42, preset='light', learning_time=5,
                       composer_params={'initial_chain':
                                            Chain(SecondaryNode('logit', nodes_from=[PrimaryNode('scaling')]))})
    ...
    prev_model.unfit()
    atomized_model = Chain(SecondaryNode(operation_type=AtomizedModel(prev_model), nodes_from=[PrimaryNode('scaling')]))
    non_atomized_model = deepcopy(prev_model)

    auto_model_from_atomized = Fedot(problem=problem, seed=42, preset='light', learning_time=5,
                                     composer_params={'initial_chain': atomized_model}, verbose_level=2)
    auto_model_from_atomized.fit(features=deepcopy(train_data.head(5000)), target='target')
    ...
    auto_model_from_chain = Fedot(problem=problem, seed=42, preset='light', learning_time=5,
                                  composer_params={'initial_chain': non_atomized_model}, verbose_level=2)
    auto_model_from_chain.fit(features=deepcopy(train_data.head(5000)), target='target')
