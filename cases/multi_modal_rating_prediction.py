...
train, test = train_test_data_setup(data, shuffle_flag=False)
...
train_img, test_img = train_test_data_setup(data_img, shuffle_flag=False)

data_text = InputData.from_json_files(path, fiedls_to_use=['plot'],
                                      label='rating', task=task)
data_text.target = class_labels
train_text, test_text = train_test_data_setup(data_text, shuffle_flag=False)
